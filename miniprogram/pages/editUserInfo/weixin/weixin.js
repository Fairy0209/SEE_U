...
Page({
  ...
  onReady: function () {
    this.setData({
      weixinNumber:app.userInfo[0].weixinNumber
    });
  },
  ...
  handleText:function(ev){
    ...
    console.log(this.data.weixinNumber)
  },
  upDateweixinNumber:function(){
    wx.showLoading({
      title: '更新中',
    })
    if(app.userInfo){
      db.collection("users").doc(app.userInfo[0]._id).update({
        data:{
          ...
          app.userInfo[0].weixinNumber = this.data.weixinNumber;
      })
    }
